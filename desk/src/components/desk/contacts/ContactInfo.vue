			...
			<div class="flex flex-col space-y-[24px]">
				<div>
					...
					<div class="flex flex-row space-x-[8px] items-center">
						<FileUploader @success="(file) => setContactImage(file.file_url)">
							<template v-slot="{
								file,
								progress,
								uploading,
								...
								<div class="flex items-center space-x-2">
									<UserAvatar size="lg" :fullName="values?.contactName"
									...
									<Button v-if="values?.profilePicture" @click="setContactImage(null)">
										Remove
									</Button>
								</div>
							</template>
				...
				<Input :readonly="!editingName" class="grow" label="E-mail" type="text" :value="values?.email"
					@change="(val) => (values.email = val)" />
				<Input :readonly="!editingName" class="grow" label="Phone" type="text" :value="values?.phone"
					@change="(val) => (values.phone = val)" />
				<Input :readonly="!editingName" class="grow" label="Organization" v-model="organization" type="text" :value="values?.organization" />
				<div class="w-full flex flex-row">
					...
					<div class="grow flex flex-row-reverse">
						<Button :loading="this.$resources.contact.setValue.loading" appearance="primary"
...
export default {
			...
			return {
				...
				phone:
					this.contactDoc && this.contactDoc.phone_nos.length > 0
						...
						: "",
				organization: this.contactDoc?.organization || null
			}
		},
	},
	deactivated() {
	...
	resources: {
		...
		delete_contacts() {
			return {
				method: "frappedesk.api.doc.delete_single_item",
				onSuccess: () => {
					this.$toast({
